...
#
#         # ------------------------------------------------------------------
#         # [安全检查] 再次确认 idx 是否是字牌 (27-33)
#         # 如果是字牌，这里绝对不能进入 B.顺子 逻辑
#         # ------------------------------------------------------------------
#
#         # A. 刻子 (AAA) - 字牌可以做刻子
#         if hand[idx] + laizi >= 3:
#             used_laizi = max(0, 3 - hand[idx])
#             orig = hand[idx]
#             hand[idx] = max(0, hand[idx] - 3)
#             if FengHuaRules._check_melds(hand, laizi - used_laizi, idx):
#                 hand[idx] = orig
#                 return True
#             hand[idx] = orig
#
#         # B. 顺子 (ABC) - 严防死守：字牌(>=27)绝对不能进这里
#         if idx < 27 and (idx % 9) <= 6:
#             # 需要 idx, idx+1, idx+2 各一张
#             c2 = 1 if hand[idx + 1] > 0 else 0
#             c3 = 1 if hand[idx + 2] > 0 else 0
#             needs = (1 - c2) + (1 - c3)
#
...
#
#             # ----------------------------------------------------
#             # [新增] 孤张字牌惩罚 (Isolated Honor Penalty)
#             # 目的：强行告诉 Agent，如果字牌不成对也不是赖子，就赶紧打掉
#             # ----------------------------------------------------
#             # 遍历字牌 (ID 27-33)
#             for h_id in range(27, 34):
#                 # 如果手里有这张字牌，且只有1张，且不是赖子
#                 if player['hand'][h_id] == 1 and h_id not in self.laizi_set:
#                     reward -= 2  # 小惩罚，逼它打出去
#
#         return self.get_observation(self.current_player), reward, done, info
