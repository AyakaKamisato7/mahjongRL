...
def save_game_replay(env, episode_id, winner_id, reward, final_hand_type):
    """
    [功能] 保存回放，输出每一轮动作以及当时所有 Agent 的手牌
    """
    ...
    with open(filepath, "w", encoding="utf-8") as f:
        ...
        for step_i, record in enumerate(env.action_history):
            pid = record['pid']
            act = record['action']
            snapshot = record['snapshot']  # 这是一个 list，包含 4 个玩家的状态字典
            ...
            for p_i in range(4):
                ...
                for t_id, count in enumerate(p_state['hand']):
                    if count > 0:
                        hand_str.extend([TileUtils.to_string(t_id)] * count)
        ...
        while len(agent.buffer) < Cfg.BATCH_SIZE:
            ...
            if info.get('winner') is not None:
                ...
                try:
                    ...
                    w_type = f"Err: {str(e)[:10]}"  # 错误捕获
                    # print(f"❌ Error in is_winning: {e}")

                win_types[w_type] += 1

                # 保存回放: 每 50 局保存一次，或者奖励特别高(>5)时保存
